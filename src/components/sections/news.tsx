...
export default function NewsSection({ news, isPage = false }: Props) {
  return (
    <section id="news" className="w-full flex flex-col gap-30">
      {news?.map((item) => {
        ...
        return (
          <div
            key={item.id}
            id={item.id}
            className="w-full flex flex-col lg:flex-row gap-5 items-stretch justify-stretch relative"
          >
            <Decor />
            <div className="w-full lg:w-1/2 flex items-center flex-col">
              ...
              <TextBlock
                text={croppedContent}
                options="bg-transparent text-brown"
              />
              {isCropped && (
      ...
      {!isPage && (
        <div className="w-full flex justify-center">
          <div className="w-full sm:w-2/3 lg:w-1/2 border relative">
          <PopupLink text="Все новости здесь" href={`/news`} />
        </div>
        </div>
      )}
    </section>
